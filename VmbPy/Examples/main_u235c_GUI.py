...
class CameraConfig:
    ...
    def __init__(self, duration=60):
        ...
        self.sensor_bit_depth: str = 'Bpp10'
        ...
        self.height: int = 608
        self.width: int = 968
        self.offset_x: int = 484
        self.offset_y: int = 304
...
def parse_args() -> Optional[str]:
    args = sys.argv[1:]
    if len(args) > 1:
        abort("[u235c] Invalid number of arguments. Abort.")
    ...
    with VmbSystem.get_instance() as vmb:
        if cam_id:                
            try:
                ...
                raise CameraError(f"[u235c] Failed to access camera{cam_id}")
        else:                    
            cams = vmb.get_all_cameras()
            if not cams:
                raise CameraError("[u235c] No Allied Vision cameras found")
            return cams[0]
...
"""
    with cam:
        ...
        print(
            f"[u235c] Camera settings: ExposureAuto={cfg.exposure_auto}, GainAuto={cfg.gain_auto}, ExposureTime={cfg.exposure_time}")
        ...
        try:
            ...
            while not stream.GVSPAdjustPacketSize.is_done():
                pass
            print("[u235c] Packet size adjusted")
        except (AttributeError, VmbFeatureError):
            print("[u235c] Packet size adjustment not supported")
...
def setup_pixel_format(cam: Camera):
    ...
    if opencv_display_format in cam_formats:
        ...
        abort('[u235c] Camera does not support an OpenCV compatible format. Abort.')
        ...
        if frame.get_status() == FrameStatus.Complete:
            ...
            if self.in_chunk == self.chunk_size:  #  chunk full
                try:
                    ...
                    print("[u235c] Queue FULL- block dropped")
                self.block_idx += 1
                #  new buffer 
...
def record_u235c(start_evt, stop_evt, duration, out_dir):
    try:
        cam_id = parse_args()
        ...
        with VmbSystem.get_instance():
            with get_camera(cam_id) as cam:
                ...
                print(f'[u235c] Frames captured {frame_count} valid frames')
                print(f'[u235c] Frames duration time {duration} s')
                total_time = time.time() - start_time
                print(f'[u235c] Total time {total_time}')
                fps = frame_count / duration
                print(f"[u235c] Average FPS: {fps:.2f}, Total frames: {frame_count}")
                ...
                print(f"[u235c] settings.json zapisany w  {output_folder}")
                print("[u235c] Done!")
